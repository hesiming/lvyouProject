...
public enum SimpleNetworkEngineSingleton {
   ...
   * @param domainBeanAsyncHttpResponseListener 异步网络响应监听(通过这个监听返回的代码已经处于UI线程了)
   * @return RequestHandle (可以通过这个抽屉来查看本次http请求的状态, 或者取消本次http请求)
   */
  public INetRequestHandle requestDomainBean(final Object netRequestDomainBean,
      final IDomainBeanAsyncHttpResponseListener domainBeanAsyncHttpResponseListener) {
    ...
    try {
      ...
      final IDomainBeanAbstractFactory domainBeanAbstractFactoryObject =
          DomainBeanAbstractFactoryCacheSingleton.getInstance.getDomainBeanAbstractFactoryObjectByKey(abstractFactoryMappingKey);
      ...
      final String fullUrlString =
          EngineHelperSingleton.getInstance.getSpliceFullUrlByDomainBeanSpecialPathStrategyObject().getFullUrlByDomainBeanSpecialPath(
              specialPath);
      ...
      // TODO : 添加具体接口的具体参数
      final IParseDomainBeanToDataDictionary parseDomainBeanToDataDictionary =
          domainBeanAbstractFactoryObject.getParseDomainBeanToDDStrategyObject();
      ...
      // 获取http引擎
      requestHandle =
          HttpEngineFactoryMethodSingleton.getInstance.requestDomainBean(fullUrlString, dataDictionary,
              new IDomainBeanRequestAsyncHttpResponseListener() {
                ...
                public void onFailure(final INetRequestIsCancelled netRequestIsCancelled, final int statusCode, final Throwable e) {
                  DebugLog.e(TAG, "<<<<<<<<<<     发起的 DomainBean [" + abstractFactoryMappingKey + "] 网络请求, 网络层访问失败 , 原因-->"
                  ...
                  handler.post(new Runnable() {
                    ...
                    public void run() {
                      ...
                      if (domainBeanAsyncHttpResponseListener instanceof IDomainBeanAsyncHttpResponseListenerWithUIControl) {
                        ((IDomainBeanAsyncHttpResponseListenerWithUIControl) domainBeanAsyncHttpResponseListener)
                            .onEnd(NetRequestResultEnum.Failure);
                      }
                    }
                  });
                }
                ...
                public void onSuccess(final INetRequestIsCancelled netRequestIsCancelled, final Object response) {
                  ...
                  do {
                    ...
                    final Object netUnpackedData =
                        EngineHelperSingleton.getInstance.getNetRespondEntityDataUnpackStrategyObject().unpackNetRespondRawEntityData(
                            response);
                    ...
                    error.reinitialize(EngineHelperSingleton.getInstance.getServerRespondDataTestStrategyObject()
                        .testServerRespondDataError(netUnpackedData));
                    ...
                    try {
                      final Object respondDomainBean =
                          domainBeanAbstractFactoryObject.getParseNetRespondDataToDomainBeanStrategyObject()
                              .parseNetRespondDataToDomainBean(netUnpackedData);
                      ...
                      handler.post(new Runnable() {
                        ...
                        public void run() {
                          ...
                          if (domainBeanAsyncHttpResponseListener instanceof IDomainBeanAsyncHttpResponseListenerWithUIControl) {
                            if (netRequestIsCancelled.isCancelled()) {
                              ((IDomainBeanAsyncHttpResponseListenerWithUIControl) domainBeanAsyncHttpResponseListener)
                                  .onEnd(NetRequestResultEnum.Canceled);
                            } else {
                              ((IDomainBeanAsyncHttpResponseListenerWithUIControl) domainBeanAsyncHttpResponseListener)
                                  .onEnd(NetRequestResultEnum.Succeed);
                            }
                          }
                        }
                      });
   ...
   * 
   * @param url 文件下载url
   * @param fileFullSavePath 文件完整的保存路径(包括文件名)
   * @param isNeedContinuingly 是否需要断点续传
   * @param fileAsyncHttpResponseListener 文件下载异步监听
   */
  public INetRequestHandle requestFileDownlaod(final String url, final Map<String, String> dataDictionary, final String fileFullSavePath,
      final boolean isNeedContinuingly, final IFileAsyncHttpResponseListener fileAsyncHttpResponseListener) {
    ...
    requestHandle =
        HttpEngineFactoryMethodSingleton.getInstance.requestFile(url, isNeedContinuingly, dataDictionary, file,
              ...
              public void onProgress(final long bytesWritten, final long totalSize) {
                ...
                if (fileAsyncHttpResponseListener != null) {
                  ...
                  if (fileAsyncHttpResponseListener instanceof IFileAsyncHttpResponseListenerOnProgressDoInBackground) {
                    ((IFileAsyncHttpResponseListenerOnProgressDoInBackground) fileAsyncHttpResponseListener).onProgressDoInBackground(
                        tempBytesWritten, totalSize);
                  }
                  ...
                  if (fileAsyncHttpResponseListener instanceof IFileAsyncHttpResponseListenerOnProgressDoInUIThread) {
                    handler.post(new Runnable() {
                      ...
                      @Override
                      public void run() {
                        ((IFileAsyncHttpResponseListenerOnProgressDoInUIThread) fileAsyncHttpResponseListener).onProgressDoInUIThread(
                            tempBytesWritten, totalSize);
                      }
                    });
                  }
   ...
   */
  public INetRequestHandle requestFileDownlaod(final String url, final String fileFullSavePath,
