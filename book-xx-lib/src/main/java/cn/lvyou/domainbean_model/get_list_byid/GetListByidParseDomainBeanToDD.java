...
public final class GetListByidParseDomainBeanToDD implements IParseDomainBeanToDataDictionary {
	...
	public Map<String, String> parseDomainBeanToDataDictionary(Object netRequestDomainBean) {
		...
		boolean isRightObjectType = netRequestDomainBean instanceof GetListByidNetRequestBean;
		if (!isRightObjectType) {
			throw new IllegalArgumentException("传入的业务Bean的类型不符 !");
		}

		GetListByidNetRequestBean requestBean = (GetListByidNetRequestBean) netRequestDomainBean;
		...
		// 必须参数
		params.put(GetListByidDatabaseFieldsConstant.RequestBean.id.name(), id);
