...
public class NetConnectionManageTools {
  ...
  // 即“接入点名称”，无论使用联通3G还是移动2G网络，都必须通过手机APN设置，是手机上网时必须配置的参数。
  ...
  private void checkApn() {
    ...
    // getContentResolver() : Return a ContentResolver instance for your
    // application's package.
    final Cursor cursor = GlobalDataCacheForMemorySingleton.getInstance.getApplication().getContentResolver()
        .query(PREFERRED_APN_URI, new String[] { "_id", "apn", "proxy", "port" }, null, null, null);
    ...
    do {
      ...
      if (!TextUtils.isEmpty(mProxy)) {// 如果设置了代理
        ...
        if (mApn.equalsIgnoreCase("CMWAP") || mApn.equalsIgnoreCase("UNIWAP") || mApn.equalsIgnoreCase("3GWAP")) {
          isWapNetwork = true;
          mProxy = "10.0.0.172";
          mPort = "80";
        } else if (mApn.equalsIgnoreCase("CTWAP")) {
          isWapNetwork = true;
          mProxy = "10.0.0.200";
          mPort = "80";
        }
  ...
  private void checkActiveNetworkType() {
    ...
    final ConnectivityManager connectivityManager = (ConnectivityManager) GlobalDataCacheForMemorySingleton.getInstance.getApplication().getApplicationContext()
